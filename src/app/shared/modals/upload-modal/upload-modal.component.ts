...
export class UploadModalComponent implements OnInit {
  ...
  changeType(type: string) {
    ...
    if (this.emitData) {
      ...
      this.setAttachmentTypeFlag(type, this.emitData['file']['type'], this._accept);
    } else {
      this.emitData = {
        file: null,
        name: null,
  ...
  uploadFile(event: HTMLInputEvent) {
    ...
    if (target.files && target.files[0]) {
      ...
      this.setAttachmentTypeFlag(this.selectItem.value, file.type, this._accept);
  ...
  convertBytesToMBytes(b: number): number {
    return b ? Math.round(b / Math.pow(1024, 2)) : 0;
  }
  ...
  private setAttachmentTypeFlag(selectedType: string, fileType: string, acceptedTypesList: MimeTypes[]) {
