...
export class NotificationSettingModalComponent implements OnInit, OnDestroy {
  ...
  onSave() {
    ...
    this.emitData.emit({
      valueGe:
        (!!this.settingForm.get('valueGe').value &&
          this.settingForm.get('valueGe').value.replace(/\s+/g, '').length === 0) ||
        !this.settingForm.get('valueGe').value ||
        this.settingForm.get('valueGe').value === null
          ? null
          : this.settingForm.get('valueGe').value,
      valueRu:
  ...
  private settingFormCreate() {
    this.settingForm = this.fb.group({
      valueGe: !!this.setting ? this.setting.valueGe : null,
