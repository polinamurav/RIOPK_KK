...
export class SidebarComponent implements OnChanges, OnDestroy, AfterViewInit {
  ...
  constructor(private routerURLService: RouterURLService, private tocService: TocService, private router: Router) {}
  ...
  onBack() {
    if (this.isQueueApp) {
      const prevQueueRoute = localStorage.getItem('previousRoute');
