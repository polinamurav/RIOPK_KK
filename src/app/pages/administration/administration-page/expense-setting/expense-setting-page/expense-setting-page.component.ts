...
export class ExpenseSettingPageComponent implements OnInit, OnDestroy {
  totalCount: number;
  ...
  private selectExpenseSetting$: Observable<PageDTO<AbsExpenseSetting>> = this._store.pipe(select(selectExpenseSetting));
  ...
  setOptionsList(): void {
    forkJoin([
      this.dirAccountProductService.getList(),
      this.dirExpenseTypeService.getList()
    ])
