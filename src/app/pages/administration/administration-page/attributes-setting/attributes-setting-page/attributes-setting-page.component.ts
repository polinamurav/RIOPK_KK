...
import {
  AuthState,
  PageDTO,
  PaginationAndSortingDto, ProductRes,
  TableData,
  TableDataHeader
} from "@app/_models";
...
type Options = ProductRes | DirAbsAttribute;
...
export class AttributesSettingPageComponent implements OnInit, OnDestroy {
  totalCount: number;
  ...
  private selectAttributesSetting$: Observable<PageDTO<DirAbsAttributeSetting>> = this._store.pipe(select(selectAttributesSetting));
  ...
  setOptionsList(): void {
    forkJoin([
      ...
      this.dirAbsAttributeService.getList()
    ])
