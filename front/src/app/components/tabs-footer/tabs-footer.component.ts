...
import { FOOTER_BUTTON_PRESETS } from '@app/components/tabs-footer/constants/footer-buttons-presets';
...
export class TabsFooterComponent implements OnInit {
  ...
  @Input() disabledButton: { [key: string]: boolean } = {};

  @Output() footerButtonClick: EventEmitter<FooterButtonClick> = new EventEmitter<FooterButtonClick>();

  constructor(private footerConfigs: GetTabsFooterConfigService, private credentialsService: CredentialsService) {}
  ...
  get processHistoryButtonConfig() {
    return FOOTER_BUTTON_PRESETS.processHistory;
  }
  ...
  isButtonDisabled(buttonType: string): boolean {
    ...
    if (this.disabledButton && this.disabledButton[buttonType] !== undefined) {
      return this.disabledButton[buttonType];
    }
