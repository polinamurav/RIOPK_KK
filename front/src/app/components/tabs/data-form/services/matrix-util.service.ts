...
export class MatrixUtilService {
  ...
  public isNotNullMatrixExist(matrix: BRMSMatrixDto) {
  ...
  public getTransformedMatrix(matrix: any) {
    return REF_LIABILITY_KEYS.reduce((refLiabilityObj: {}, item: RefLiabilityKey) => {
  ...
  public filterMatrix(matrix: any[], isWithRef: boolean, productId: number) {
    let filteredMatrix: any[] = matrix.filter((m: any) =>
      isWithRef
    ...
    if (!!productId) {
      filteredMatrix = filteredMatrix.filter((m: BRMSMatrixDto) => m.product.id === productId);
    }
