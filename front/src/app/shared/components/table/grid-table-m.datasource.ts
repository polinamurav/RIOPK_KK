...
import { BehaviorSubject, combineLatest, merge, Observable, of as observableOf, Subscription, Subject } from 'rxjs';
...
export class GridTableDataSource<T> extends DataSource<T> {
  ...
  set data(data: any[]) {
    ...
    this._renderData.next(this._data.slice(0, PAGESIZE));
  }
  ...
  constructor(initialData: any[], private viewport: CdkVirtualScrollViewport, private itemSize: number) {
    ...
    this.viewport.elementScrolled().subscribe((ev: any) => {
      ...
      this._renderData.next(slicedData);
    });
