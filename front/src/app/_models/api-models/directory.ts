import { ChatbotStatus, DirBusinessInspectionResult, DirRiskBasedPrice, Segment, Status, UserPosDto } from '..';
...
export class DirGoodsGroup extends BaseDir {
  id: number;
  dirGoods: Dir[];
}

export class DirAttributeFatca extends BaseDir {
  id: number;
}

export class DirFatcaStatus extends BaseDir {
  id: number;
}

export class DirAccountType extends Dir {}

export class DirFeeTermType extends Dir {}

export class Decision extends BaseDir {
  id: string;
}

export class DirConsentToProvideInformation extends BaseDir {
  id: string;
}
...
export interface DirIncomeType extends BaseDir {
  absCode?: string;
  id?: number;
  isSum?: boolean;
}
...
export class DirectoriesState {
  ...
  paymentCard: PageDTO<DirIncomeType>;
  ...
  dirFatcaRegions: PageDTO<Dir>;
  tariff: PageDTO<Dir>;
  rbp: PageDTO<DirRiskBasedPrice>;
}

export class IdentityCardType extends DirAbsCode {
  isSoc?: boolean;
}

export class SuspensiveConditionsType extends DirAbsCode {
  suspensiveConditionsTypes?: string | string[];
}

export interface AbsFirstPayDateRsDto {
  date: string; //  Дата первого платежа
  message: string; //  Сообщение, если дата не подходит
}

export class DirGoodsDtoView {
  constructor(public item: Dir | any) {
    this.setAllKeys(item);
  }

  get dirGoodsGroupsJoinRu() {
    return this.item.dirGoodsGroups.length ? this.item.dirGoodsGroups.map(e => e.nameRu).join(', ') : '';
  }

  get dirGoodsGroupsJoinAm() {
    return this.item.dirGoodsGroups.length ? this.item.dirGoodsGroups.map(e => e.nameAm).join(', ') : '';
  }

  private setAllKeys(obj: Dir) {
...
export class DirCreditProgramDtoView{

  constructor(public item: Dir | any) {
    this.setAllKeys(item);
  }

  get dirGoodsGroupsListRu() {
    return this.item.dirGoodsGroups.length ? this.item.dirGoodsGroups.map(e => e.nameRu).join(', ') : '';
  }


  get dirGoodsGroupsListAm() {
    return this.item.dirGoodsGroups.length ? this.item.dirGoodsGroups.map(e => e.nameAm).join(', ') : '';
  }
