...
export class ChatUnderManagerControllerService {
  ...
  public getAllByApplicationId(applicationId: string): Observable<CommentDto[]> {
    return this.http.get<CommentDto[]>(`${this.baseUrl}/chat-under-manager`, { params: { applicationId } });
  }
  ...
  public save(comment: CommentDto): Observable<CommentDto> {
    return this.http.post<CommentDto>(`${this.baseUrl}/chat-under-manager`, comment);
  }
