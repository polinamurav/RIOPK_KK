...
export class ApplicationControllerService {
  ...
  public getMyTasks(
    ...
    const filterParamsReq = new TransformQueryParams({
      ...
      dateFrom: filterParams.dateFrom
        ? [
          new Date(filterParams.dateFrom).getFullYear(),
          ('0' + (new Date(filterParams.dateFrom).getMonth() + 1)).slice(-2),
          ('0' + new Date(filterParams.dateFrom).getDate()).slice(-2)
        ].join('-')
        : '',
      dateTo: filterParams.dateTo
        ? [
          new Date(filterParams.dateTo).getFullYear(),
          ('0' + (new Date(filterParams.dateTo).getMonth() + 1)).slice(-2),
          ('0' + new Date(filterParams.dateTo).getDate()).slice(-2)
        ].join('-')
        : '',
      type
    });
    ...
    return this.http
      .post<PageDTO<ApplicationPagedInfoDto>>(`${this.baseUrl}/applications/queue/${type}`, filterParamsReq, { params })
      .pipe(
        map(response => {
          if (response.content) {
            response.content = response.content.map(item => ({
              ...item,
              dirTradingCompanyPointFull:
                item.dirTradingCompanyPointNameAm && item.dirTradingCompanyPointAddress
                  ? `${item.dirTradingCompanyPointNameAm}. ${item.dirTradingCompanyPointAddress}`
                  : null
            }));
          }
   ...
   */
  public acceptApp(applicationId: string | number, lang: string, isOtp: boolean = false): Observable<string> {
    const params = { applicationId, lang, isOtp };
   ...
   * @param dirDecisionMakerTypeDecisionId   для запроса
   * @returns Observable of ApplicationDto
   */
  public setDecisionMakerDecision(
    applicationId: string | number,
    decisionMakerDecisionId: string | number,
    dirDecisionMakerTypeDecisionId: string | number
  ): Observable<ApplicationDto> {
    return this.http.post<ApplicationDto>(
      `${
        this.baseUrl
      }/applications/${applicationId}/setDecisionMakerDecision/${decisionMakerDecisionId}/${dirDecisionMakerTypeDecisionId}`,
      null
    );
  }

  public saveRmLimitPos(rmLimitPosDto: RmLimitPosDto): Observable<ApplicationDto> {
    return this.http.post<ApplicationDto>(`${this.baseUrl}/rm-limit-pos`, rmLimitPosDto);
  }

  public setVerifierDeclineReason(
    applicationId: string | number,
    opzDecisionDto: OpzDecisionDto
  ): Observable<ApplicationDto> {
    return this.http.post<ApplicationDto>(
      `${this.baseUrl}/applications/${applicationId}/setVerifierDeclineReason`,
      opzDecisionDto
    );
  }
  ...
  // Получение сообщения о причине возврата
  public applicationsReturnCheck(applicationId: string | number) {
    return this.http.get(`${this.baseUrl}/applications/return-check/${applicationId}`, {
      responseType: 'text'
    });
  }

  // Получение сообщения об ошибке ЭКЕНГ
  public ekengErrorCheck(applicationId: string | number) {
    return this.http.get<any>(`${this.baseUrl}/ekeng/message/${applicationId}`);
  }

  // Получение сообщения об ошибке по заявке
  public applicationGetErrorMessage(applicationId: string | number): Observable<any> {
    return this.http.get<string>(`${this.baseUrl}/applications/${applicationId}/error-message`);
  }

  public applicationsType(): Observable<string[]> {
    return this.http.get<string[]>(`${this.baseUrl}/applications/type`);
  }
}
