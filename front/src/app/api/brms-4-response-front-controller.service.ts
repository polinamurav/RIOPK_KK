...
export class Brms4ResponseFrontControllerService {
  ...
  public getAll(): Observable<BRMSResponseDto[]> {
    return this.http.get<BRMSResponseDto[]>(`${this.baseUrl}/brms4-response`);
  }
  ...
  public getById(id: number | string): Observable<BRMSResponseDto> {
    return this.http.get<BRMSResponseDto>(`${this.baseUrl}/brms4-response/${id}`);
  }
}
