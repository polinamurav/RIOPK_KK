...
import { PreapprovedOfferControllerService } from '@app/api/preapproved-offer.controller.service';
...
export class AdministrationEffects {
  ...
  // Preapproved
  getApprovedOfferAction$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(AdministrationActions.AdmSetPreapprovedOfferAction),
      switchMap(({ data, paginationData }) => {
        return this.preapprovedOfferService.getByPage(data.shortApplicationId, paginationData as PaginationAndSortingDto);
      }),
      switchMap(data => {
        return of(AdministrationActions.AdmSetPreapprovedOfferSuccessAction({ data }));
      })
    );
  });
  ...
  getCompany$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap(({ data }) => {
        return this.companyService.getCorpCompanyByPage(data as PaginationAndSortingDto);
      }),
      switchMap(data => {
  ...
  updateCorpCompany$ = createEffect(() => {
    let paginData: PaginationAndSortingDto;
    return this.actions$.pipe(
      ofType(AdministrationActions.AdmUpdateCorpCompanyAction),
      switchMap(({ data, paginationData }) => {
        paginData = paginationData;
        return this.companyService.updateCorpCompany(data);
      }),
      switchMap(() => {
        return of(AdministrationActions.AdmSetCompanyListAction({ data: paginData }));
      })
  ...
  // levels pm
  getProductTemplate$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(AdministrationActions.ProductTemplateAction),
      switchMap(() => {
        return this.productTemplateService.getList();
      }),
      switchMap(data => {
        return of(AdministrationActions.ProductTemplateSuccessAction({ data }));
      })
    );
  });

  // levels pm
  getLevelsPM$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(AdministrationActions.AdmLevelsPMAction),
      switchMap(() => {
        return this.dirLevelsPmService.getList();
      }),
      switchMap(data => {
        return of(AdministrationActions.AdmLevelsPMSuccessAction({ data }));
      })
    );
  });

  createLevelsPM$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(AdministrationActions.AdmCreateLevelsPMAction),
      switchMap(({ data }) => {
        return this.dirLevelsPmService.create(data);
      }),
      switchMap(() => {
        return of(AdministrationActions.AdmLevelsPMAction(null));
      })
    );
  });

  updateLevelsPM$ = createEffect(() => {
    return this.actions$.pipe(
      ofType(AdministrationActions.AdmLevelsPMUpdateAction),
      switchMap(({ data }) => {
        return this.dirLevelsPmService.update(data);
      }),
      switchMap(() => {
        return of(AdministrationActions.AdmLevelsPMAction(null));
      })
  ...
  constructor(
    ...
    private preapprovedOfferService: PreapprovedOfferControllerService,
