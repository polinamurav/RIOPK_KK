...
import { RuleGroupControllerService } from '@app/api/massegment-api/product-group-controller.service copy';
...
export interface RuleData {
  ...
  dirRuleGroupList?: BRMSRuleGroupDto[];
  ruleData?: BRMSRule;
...
export class RulesPageComponent extends AllowEditDirective implements OnInit, OnDestroy {
  ...
  objColNameProps: TableDataHeader[] = [
    ...
    new TableDataHeader('nameAm', 'Administration.TableHeaders.RuleName', 'am', 'nameAm'),
    new TableDataHeader('brmsRuleGroup.id', 'Administration.TableHeaders.RuleGroup', 'string', 'brmsRuleGroup.id'),
  ...
  constructor(
    ...
    private ruletGroupService: RuleGroupControllerService
  ) {
    ...
    this.showDialog(
      {
        ...
        dirRuleGroupList: this.ruleGroupList,
        disabledFields: true,
        ruleData
      },
      (val: BRMSRuleDTO) => {
  ...
  private getRuleGroupList() {
    this.ruletGroupService
      .getList()
      .pipe(takeUntil(this.destroy$))
      .subscribe(res => {
