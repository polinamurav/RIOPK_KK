...
import { AuthState, PageDTO, PaginationAndSortingDto, RoleAuthority, TableData, TableDataHeader } from '@app/_models';
...
export class AccountProductPageComponent extends AllowEditDirective implements OnInit, OnDestroy {
  totalCount: number;
  ...
  private selectAccountProduct$: Observable<PageDTO<DirAccountProduct>> = this._store.pipe(
    select(selectAccountProduct)
  );
